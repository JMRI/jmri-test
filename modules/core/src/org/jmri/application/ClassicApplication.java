...
public class ClassicApplication extends JPanel implements PropertyChangeListener, java.awt.event.WindowListener {
    ...
    static org.slf4j.Logger log = LoggerFactory.getLogger(ClassicApplication.class);

    public ClassicApplication(JFrame frame) {
        super(true);
            ...
            try {
                if (ProfileManager.defaultManager().migrateToProfiles(configFilename)) { // migration or first use
                    ...
                    JOptionPane.showMessageDialog(sp,
                            AppsBundle.getMessage("ConfigMigratedToProfile"),
    ...
    protected String line1() {
        return AppsBundle.getMessage("DefaultVersionCredit", jmri.Version.name());
    }
    ...
    protected void updateLine(ConnectionConfig conn, JLabel cs) {
        ...
        if (ConnectionStatus.instance().isConnectionOk(conn.getInfo())) {
            ...
            String cf = AppsBundle.getMessage("ConnectionSucceeded", name, conn.name(), conn.getInfo());
            cs.setText(cf);
        } else {
            cs.setForeground(Color.red);
            String cf = AppsBundle.getMessage("ConnectionFailed", name, conn.name(), conn.getInfo());
            cf = cf.toUpperCase();
            cs.setText(cf);
        }
    ...
    protected String line9() {
        return AppsBundle.getMessage("JavaVersionCredit",
    ...
    static public void initLogging() {
        ...
        log = LoggerFactory.getLogger(ClassicApplication.class);
        // first log entry
        log.info(jmriLog);
    ...
    static public void createFrame(ClassicApplication containedPane, JFrame frame) {
